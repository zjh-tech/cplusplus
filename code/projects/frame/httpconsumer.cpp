 ...
 * @Descripttion: 
 * @Author: zhengjinhong
 * @Date: 2021-01-18 19:02:14
 * @LastEditors: zhengjinhong
 * @LastEditTime: 2021-01-21 16:54:19
 */
...
bool HttpConsumer::Init() {
  thread_ptr = make_shared<thread>([this]() {
    run();
  });
  return true;
}
...
void HttpConsumer::run() {
  LogInfo("Start HttpConsumer Thread");
  bool busy = false;
  while (!terminate) {
    busy = false;
  ...
  LogInfo("End HttpConsumer Thread");
}