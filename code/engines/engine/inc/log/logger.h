    ...
    namespace Log
    {
        ...
        public:
            ...
            void AddLogEvent(eLogLevel level, const char* fmt, const Args&... args)
            {
                ...
                lock_guard<mutex> lock_guard(log_mutex_);
        ...
        private:
            ...
            void out_put(eLogLevel level, const string& content);

            bool check_diff_date(std::tm* tm_last_ptr, std::tm* tm_next_ptr);

            bool ensure_file_exist(std::tm* tm_next_ptr);
