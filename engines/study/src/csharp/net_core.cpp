    ...
    namespace CSharp
    {
            ...
            if (m_coreclr)
            {
                if (m_handle && m_domain_id != 0)
                {
                    ...
                    coreclr_shutdown_ptr coreclr_shutdown =
                        reinterpret_cast<coreclr_shutdown_ptr>(dlsym(m_coreclr, "coreclr_shutdown"));
#else
                    coreclr_shutdown_ptr coreclr_shutdown =
                        (coreclr_shutdown_ptr)GetProcAddress(m_coreclr, "coreclr_shutdown");
#endif
                    if (coreclr_shutdown)
                    {
        ...
        bool CNetCore::Init(const char* app_path, const char* pkg_path)
        {
            ...
            coreclr_initialize_ptr coreclr_init =
                reinterpret_cast<coreclr_initialize_ptr>(dlsym(m_coreclr, "coreclr_initialize"));
            m_coreclr_create_delegate =
                reinterpret_cast<coreclr_create_delegate_ptr>(dlsym(m_coreclr, "coreclr_create_delegate"));
#else
            ...
            coreclr_initialize_ptr coreclr_init =
                reinterpret_cast<coreclr_initialize_ptr>(GetProcAddress(m_coreclr, "coreclr_initialize"));
            m_coreclr_create_delegate =
                reinterpret_cast<coreclr_create_delegate_ptr>(GetProcAddress(m_coreclr, "coreclr_create_delegate"));
#endif
            ...
            int error_code = coreclr_init(app_path,                                 // exePath
        ...
        bool CNetCore::GetCSharpProcAddress(const char* name_space, const char* class_name, const char* func_name,
                                            void** out_delegate)
        {
            ...
            int error_code =
                m_coreclr_create_delegate(m_handle, m_domain_id, name_space, class_name, func_name, out_delegate);
