...
public:
  ...
  void test_min() {
    ...
    timer_register->AddOnceTimer(
      eTimerID::EVERY_SOME_MIN_CLOCK, min_timestamp, [min, this]() {
        timer_min();

        timer_register->AddRepeatTimer(eTimerID::EVERY_SOME_MIN_CLOCK, ONE_MIN_MILL_MACRO * min, [this]() {
          timer_min();
        });
      });
  }
  ...
  void test_hour() {
    ...
    timer_register->AddOnceTimer(
      eTimerID::EVERY_SOME_HOUR_CLOCK, hour_timestamp, [hour, this]() {
        timer_hour();

        timer_register->AddRepeatTimer(eTimerID::EVERY_SOME_HOUR_CLOCK, ONE_HOUR_MILL_MACRO * hour, [this]() {
          timer_hour();
        });
      });
  }
  ...
  void test_wday() {
    ...
    timer_register->AddOnceTimer(
      eTimerID::EVERY_SOME_WDAY_CLOCK, wday_timestamp, [wday, this]() {
        timer_wday();

        timer_register->AddRepeatTimer(eTimerID::EVERY_SOME_WDAY_CLOCK, ONE_DAY_MILL_MACRO * 7, [this]() {
          timer_wday();
        });
      });
  }
  ...
  void test_month() {
    ...
    timer_register->AddOnceTimer(
      eTimerID::EVERY_SOME_MONTH_CLOCK, month_timestamp, [this]() {
        ...
        timer_register->AddOnceTimer(eTimerID::EVERY_SOME_MONTH_CLOCK, ONE_MONTH_MILL_MACRO(tatal_day_count), [this]() {
          timer_month();
        });
      });
  }
...
int main(void) {
  ...
  LOG_INIT("./timer_log", "timer_example", false, 1);
#else
  LOG_INIT("./timer_log", "timer_example", true, 1);
#endif
